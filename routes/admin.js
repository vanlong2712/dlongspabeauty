      ...
      if (user) {
        ...
        newUser.save().then(async user => {
          ...
          var html = `XinhXinhSalon cung cấp tài khoản truy cập quyền quản trị: <br /><br />
                          ...
                          Vui lòng nhấn vào <a href='${baseURL}/187_admin/admin-activeCode/${user._id}/${user.activeCode}' >đây</a> để khởi động tài khoản.`;
          var object = {
            toEmail: user.email,
            subject:
              'XinhXinhSalon: Cung cấp tài khoản truy cập quyền quản trị',
            text: text,
            html: html
          };
...
router.get('/admin-active/:id/:boolean', async (req, res, next) => {
  ...
  var doc = await dbUser.findOneAndUpdate(
    { _id: id },
